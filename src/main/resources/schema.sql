...
COMMENT ON TABLE film IS 'Список фильмов';
COMMENT ON COLUMN film.name IS 'Название фильма';
COMMENT ON COLUMN film.description IS 'Описание содержимого фильма';
COMMENT ON COLUMN film.release_date IS 'Дата релиза';
COMMENT ON COLUMN film.duration IS 'Продолжительность фильма в минутах';
COMMENT ON COLUMN film.rating_id IS 'ИД рейтинга';

CREATE TABLE genres IF NOT EXISTS genres
(
...
COMMENT ON COLUMN genres IS genres.genre_name IS 'Имя жанра';
...
COMMENT ON COLUMN friends.user_id IS 'ИД пользователя подавшего заявку';
